import { Engine, Graphviz } from "@hpcc-js/wasm-graphviz";
...
export const GraphvizProvider: FC<{ children: ReactNode }> =
  ({ children }) => {
  const [graphviz, setGraphviz] = useState<Graphviz>(null);
    ...
    return <GraphVizContext.Provider value={graphviz}>{children}</GraphVizContext.Provider>;
  };
...
export const useGraphviz = ()  => {
  ...
  const api = useMemo<GraphvizAPI>(() => ({
    ...
    renderToSVG: (dot: string, engine: Engine) => wasm?.layout(dot, 'svg', engine),
  }), [wasm]);
  return api;
};
