...
**Playground:**
 ...
 <PreviewTSGraphvizScript
  title="Type hint preview of ts-graphviz"
  script={`
import { digraph, attribute as _ } from 'ts-graphviz';

export default digraph('G', (g) => {
...
:::tip
Press the **RUN PROJECT** button to start the editor.

When the editor starts and the npm install process is complete, hover over `digraph` or `g.node` or `[_.color]` for type hints.
:::
